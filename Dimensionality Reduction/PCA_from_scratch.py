...
plt.bar(
    ...
    alpha=0.5,
    align="center",
    label="Induvivual variance Explained",
)

plt.step(
    range(1, 14), cumulative_var_exp, where="mid", label="Cumulative Explained Variance"
)
...
eigen_pairs = [
    (np.abs(eigen_vals[i]), eigen_vectors[:, i]) for i in range(len(eigen_vals))
]
